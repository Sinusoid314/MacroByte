...
//Then end the runtime process.
{
    ...
    if(errorFlag)
    {
        MessageBox(NULL, "The error subroutine failed to run correctly.", "MacroByteRT", MB_OK | MB_ICONERROR | MB_SETFOREGROUND);
        ExitProcess(0);
    }
    ...
    else
    {
        ...
        MessageBox(NULL, errorMsg.c_str(), "MacroByteRT", MB_OK | MB_ICONERROR | MB_SETFOREGROUND);
    }
