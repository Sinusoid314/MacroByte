...
//
{
    ...
    GetModuleFileName(NULL, tmpPath, _MAX_PATH);
    ...
    //Load version string
    tmpStr = lineList[listPos].c_str();
    listPos++;
    if(tmpStr != versionStr)
    {
        //ERROR
        MessageBox(NULL, "Wrong version number", "MacroByteRT", MB_OK | MB_ICONERROR);
        ExitProcess(0);
    }
    ...
    #endif
        //ERROR
        MessageBox(NULL, "Incorrect runtime mode.", "MacroByteRT", MB_OK | MB_ICONERROR);
        ExitProcess(0);
    }
...
void ExtractRunData(string& dataBuffer)
{
    ...
    {
        MessageBox(NULL, "Failed to open debug file.", "MacroByteRT", MB_OK | MB_ICONERROR);
        ExitProcess(0);
    }
#endif
