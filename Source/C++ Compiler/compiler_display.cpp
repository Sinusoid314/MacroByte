...
//display window
{
    ...
    hDispWin = CreateWindowEx(0,dispWndClassName, "Compile Prograss",
...
//its window class
{
    ...
    UnregisterClass(dispWndClassName,(HINSTANCE)GetModuleHandle(NULL));   
}
...
LRESULT CALLBACK DisplayWinProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_CREATE:
            ...
            hDispCancelBtn = CreateWindowEx(0, "BUTTON", "Cancel", WS_VISIBLE|WS_CHILD,
                                            1, 1, 1, 1, hwnd, NULL,
                                            (HINSTANCE) GetModuleHandle(NULL), NULL);
            hDispStatus = CreateWindowEx(0, "STATIC", "Compiling some stuff...", WS_VISIBLE|WS_CHILD|SS_CENTER,
                                            1, 1, 1, 1, hwnd, NULL,
                                            (HINSTANCE) GetModuleHandle(NULL), NULL);
            hDispProgress = CreateWindowEx(0, PROGRESS_CLASS, "", WS_VISIBLE|WS_CHILD|PBS_SMOOTH,
                                            1, 1, 1, 1, hwnd, NULL,
                                            (HINSTANCE) GetModuleHandle(NULL), NULL);
            break;
